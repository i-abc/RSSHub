                ...
                items: $('.job-box ul.list li .left .job')
                    ...
                    .map((item) => {
                        ...
                        return {
                            ...
                            description: '公司:' + item.find('.company a').text().trim() + ' ' + '薪资:' + item.find('.salary p').text().trim() + ' ' + '学历:' + item.find('.salary ul li').eq(2).text().trim() + ' ' + '性质:' + item.find('.salary ul li').eq(1).text().trim() + ' ' + '地点:' + item.find('.salary ul li').eq(0).text().trim(),  // 文字摘要
                        };
                    }),
            };
        },
